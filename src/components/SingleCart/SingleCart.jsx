...
const SingleCart = ({cartData}) => {
  ...
  return (
    <div className='border rounded mb-10'>
      ...
      <button onClick={()=>setIsSubscribe()} className={`btn w-full font-bold ${isSubscribe === false? 'bg-gray-500': 'bg-red-500'}`}>{isSubscribe === false? 'Subscribe Now' : 'Subscribed'}</button>
      </div>
    </div>
  );
};
